...
class AdvancedStockAnalyzer:
    ...
    def _create_control_panel(self, parent):
        ...
        interval_combo = ttk.Combobox(top_row, textvariable=self.interval_var, width=8,
                                      values=["1m", "2m", "5m", "15m", "30m", "60m", "90m", "1h", "1d", "5d", "1wk",
                                              "1mo", "3mo"])
    ...
    def _load_data(self):
        try:
            ...
            self.root.after(0, lambda: self.status_label.config(
                text=f"Loaded {ticker} - {len(self.current_data)} records"))
    ...
    def _add_to_portfolio(self):
        ...
        raw_ticker = self.ticker_var.get().strip().upper()
        ticker = raw_ticker if '.' in raw_ticker else raw_ticker + ".NS"
        ...
        try:
            ...
            if np.isnan(current_price):
                messagebox.showerror("Error", f"Invalid price data for {ticker}")
                return
    ...
    def _update_portfolio_chart(self):
        ...
        filtered = [
            (ticker, self.portfolio[ticker]['shares'] * self.portfolio[ticker]['price'])
            for ticker in self.portfolio
            if not np.isnan(self.portfolio[ticker]['price'])
        ]

        if not filtered:
            ax1.text(0.5, 0.5, "No valid portfolio data", ha='center', va='center', color='white', fontsize=14)
